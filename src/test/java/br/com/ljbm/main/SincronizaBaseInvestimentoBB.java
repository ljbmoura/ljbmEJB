...
import br.com.ljbm.fp.LeitorExtratoInvestimentosBBTest;
...
public class SincronizaBaseInvestimentoBB {
...
//		objectMapper.enable(MapperFeature.SORT_PROPERTIES_ALPHABETICALLY);
	...
	public static void main(String[] args) throws IOException {
		...
		log = LogManager.getFormatterLogger(SincronizaBaseInvestimentoBB.class);
...
//		List<PosicaoTituloPorAgente> extrato = 
		List<ExtratoInvestimento> extratoInvestimento=new LeitorExtratoInvestimentosBB().analisador(
//					.caminhoArquivoExtratoTD(
							Recurso.getPastaRecursos(LeitorExtratoInvestimentosBBTest.class)
							+ File.separator +
							"extratoInvestimentosAtual.txt")
					;
		
		extratoInvestimento.stream().forEach(posicao -> {
			
			FundoInvestimento ref = new FundoInvestimento();

			FundoInvestimento fundo = buscaFundoInvestimento(
					"BB BANCO DE INVESTIMENTO S/A"
					, posicao.getFundoInvestimento().getNome());
			...
			posicao.getAplicacoes().forEach(compra -> {
